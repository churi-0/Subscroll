...
const snapshot=()=>({v:6,mediaOnly:S.mediaOnly,blur:S.blur,info:S.info,muted:S.muted,
  groups:S.groups,activeId:S.activeId===TMP?FAV:S.activeId});
const save=()=>{ try{ localStorage.setItem(LS,JSON.stringify(snapshot())); }catch(e){} };
function applySettings(d){ if(hydrate(d)){ normalise(); save(); } }
